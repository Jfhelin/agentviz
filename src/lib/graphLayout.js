...
import { formatCacheUsageSummary, summarizeTokenUsage } from "./cacheMetrics";
...
export function buildTurnSnippet(turn, turnEvents) {
  ...
  var tokenUsage = summarizeTokenUsage(turnEvents.map(function (entry) {
    return entry.event && entry.event.tokenUsage;
  }));
  ...
  if (tokenUsage && tokenUsage.cacheRead > 0) {
    return formatCacheUsageSummary(tokenUsage, { variant: "verbose" });
  }
