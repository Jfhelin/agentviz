...
function buildReportInstructions(planMode: boolean): string {
  ...
  lines.push("shown. The hello-world framing already lives in **Core story** —");
  lines.push("here, just confirm the attribution with the pre-vs-post-divergence");
  lines.push("split and the prefix-tax projection (\"prefix-tax projection over");
  lines.push("N calls is +X cr, which matches the actual +Y cr delta within");
  lines.push("Z cr\"). Do not claim the cheaper run is \"better\" — Output");
  lines.push("quality and Artifact outcome answered that. Do not re-state the");
  lines.push("headline number more than twice in the whole report.");
  lines.push("");
